    ...
    public static class GeneralUtils {
        ...
        public static FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging) {
            ...
            return FunctionUpdater.Create(() => {
                ...
                if (dragging) {
                    onMouseDragging(GeneralUtils.GetMouseWorldPosition());
                }
                return false; 
            });
        }
        ...
        public static FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition) {
            ...
            return FunctionUpdater.Create(() => {
                if (state == 1) {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, GeneralUtils.GetMouseWorldPosition());
                }
                if (state == 1 && Input.GetMouseButtonDown(cancelMouseButton)) {
                    // Cancel
                    state = 0;
                }
                if (Input.GetMouseButtonDown(mouseButton) && !GeneralUtils.IsPointerOverUI()) {
                    if (state == 0) {
                        state = 1;
                        from = GeneralUtils.GetMouseWorldPosition();
                    } else {
                        state = 0;
                        onMouseClickFromTo(from, GeneralUtils.GetMouseWorldPosition());
                    }
                }
                return false; 
            });
